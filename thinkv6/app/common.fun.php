...
function get_vul_cn_name($id)
{
    ...
    $fs = Db::table('name_cn')->where(['id' => $id])->find();
    return $fs['name_cn'];
}
...
function specify_server()
{
    ...
    $results = Db::table(point_server)->order("level")->find();
    $hash = $results['hash'];
...
function point_display()
{
    ...
    $results = Db::table('point_server')->select()->toArray();
    if ($results) {
...
function customer_display()
{
    ...
    $results = Db::table('customer')->select()->toArray();
    if ($results) {
        foreach ($results as $i => $fs) {
            $id = $fs["id"];
            $name = $fs["name"];
...
function scan_display()
{
    ...
    if ($results) {
        foreach ($results as $fs) {
            ...
            foreach($results1 as $fs1){
                $customer_str .= '<option value="' . $fs1['id'] . '">' . $fs1['name'] . '</option>';
            }
            $customer_str .= '</select>';
...
function info_display()
{
    ...
    if ($results) {
        foreach ($results as $i => $fs) {
            ...
            foreach ($results2 as $fs2){
                $customer_str .= '<option value="' . $fs2['id'] . '">' . $fs2['name'] . '</option>';
            }
            $customer_str .= '</select>';
...
function set_display()
{
    ...
    $results = Db::table('user')->select()->toArray();
    if ($results) {
        foreach ($results as $i => $fs) {
