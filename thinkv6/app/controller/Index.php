...
class Index extends BaseController
{
    ...
    function spider_index()
    {
        ...
        if ($results) {
            foreach ($results as $i => $fs) {
                $id = $i;
                $url = substr($fs["url"], 0, 40);
                $link_url = $fs["link_url"];
                $customer = $fs["name"];
                $status = $fs["status"];
                $hash = $fs["hash"];
                $url_num = $fs["url_num"];
...
//        $in_arr['status'] = 'new';
        ...
        $totalnum = Db::table("scan_list")->count();
        $pagesize = 100;
        //总共有几页
    ...
    function proxy()
    {
        ...
        if ($results) {
            foreach ($results as $i => $fs) {
                ...
                $failure_times = $fs["failure_times"];
                $response_time = $fs["avg_response_time"];
                $score = $fs["score"];
